export const apiUrl = 'http://localhost:5000'

//menu
export const MENU_LOADED_SUCCESS = 'MENU_LOADED_SUCCESS'
export const MENU_LOADED_FAIL = 'MENU_LOADED_FAIL'
export const CLEAR_ORDER = 'CLEAR_ORDER'
export const FIND_MENU = 'FIND_MENU'
export const ADD_ORDER = 'ADD_ORDER'
export const UPDATE_ORDER = 'UPDATE_ORDER'

//face
export const token = '4740ae6347b0172c01254ff55bae5aff5199f4446e7f6d643d40185b3f475145'
export const DETECT_FACE = 'DETECT_FACE'
export const DETECT_FACE_FAIL = 'DETECT_FACE_FAIL'
export const GET_FACEDATA = 'GET_FACEDATA'
export const DETECTING = 'DETECTING'
export const GET_CUSTOMER_ORDER = 'GET_CUSTOMER_ORDER'
export const GET_CUSTOMER_RECOMMENDATION = 'GET_CUSTOMER_RECOMMENDATION'
export const CLEAR_CUSTOMER_RECOMMENDATION = 'CLEAR_CUSTOMER_RECOMMENDATION'

//user
export const SET_AUTH = 'SET_AUTH'
export const LOCAL_STORAGE_TOKEN_NAME = 'smart-coffeeshop'